# This script segment is generated automatically by AutoPilot

# clear list
if {${::AESL::PGuard_autoexp_gen}} {
    cg_default_interface_gen_dc_begin
    cg_default_interface_gen_bundle_begin
    AESL_LIB_XILADAPTER::native_axis_begin
}

# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 446 \
    name Q_out_real \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_real \
    op interface \
    ports { Q_out_real_address0 { O 3 vector } Q_out_real_ce0 { O 1 bit } Q_out_real_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_real'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 447 \
    name Q_out_real_1 \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_real_1 \
    op interface \
    ports { Q_out_real_1_address0 { O 3 vector } Q_out_real_1_ce0 { O 1 bit } Q_out_real_1_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_real_1'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 448 \
    name Q_out_real_2 \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_real_2 \
    op interface \
    ports { Q_out_real_2_address0 { O 3 vector } Q_out_real_2_ce0 { O 1 bit } Q_out_real_2_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_real_2'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 449 \
    name Q_out_real_3 \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_real_3 \
    op interface \
    ports { Q_out_real_3_address0 { O 3 vector } Q_out_real_3_ce0 { O 1 bit } Q_out_real_3_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_real_3'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 450 \
    name Q_out_imag \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_imag \
    op interface \
    ports { Q_out_imag_address0 { O 3 vector } Q_out_imag_ce0 { O 1 bit } Q_out_imag_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_imag'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 451 \
    name Q_out_imag_1 \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_imag_1 \
    op interface \
    ports { Q_out_imag_1_address0 { O 3 vector } Q_out_imag_1_ce0 { O 1 bit } Q_out_imag_1_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_imag_1'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 452 \
    name Q_out_imag_2 \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_imag_2 \
    op interface \
    ports { Q_out_imag_2_address0 { O 3 vector } Q_out_imag_2_ce0 { O 1 bit } Q_out_imag_2_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_imag_2'"
}
}


# XIL_BRAM:
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc ::AESL_LIB_XILADAPTER::xil_bram_gen] == "::AESL_LIB_XILADAPTER::xil_bram_gen"} {
eval "::AESL_LIB_XILADAPTER::xil_bram_gen { \
    id 453 \
    name Q_out_imag_3 \
    reset_level 1 \
    sync_rst true \
    dir I \
    corename Q_out_imag_3 \
    op interface \
    ports { Q_out_imag_3_address0 { O 3 vector } Q_out_imag_3_ce0 { O 1 bit } Q_out_imag_3_q0 { I 32 vector } } \
} "
} else {
puts "@W \[IMPL-110\] Cannot find bus interface model in the library. Ignored generation of bus interface for 'Q_out_imag_3'"
}
}


# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id 440 \
    name memQ \
    type other \
    dir O \
    reset_level 1 \
    sync_rst true \
    corename dc_memQ \
    op interface \
    ports { m_axi_memQ_AWVALID { O 1 bit } m_axi_memQ_AWREADY { I 1 bit } m_axi_memQ_AWADDR { O 64 vector } m_axi_memQ_AWID { O 1 vector } m_axi_memQ_AWLEN { O 32 vector } m_axi_memQ_AWSIZE { O 3 vector } m_axi_memQ_AWBURST { O 2 vector } m_axi_memQ_AWLOCK { O 2 vector } m_axi_memQ_AWCACHE { O 4 vector } m_axi_memQ_AWPROT { O 3 vector } m_axi_memQ_AWQOS { O 4 vector } m_axi_memQ_AWREGION { O 4 vector } m_axi_memQ_AWUSER { O 1 vector } m_axi_memQ_WVALID { O 1 bit } m_axi_memQ_WREADY { I 1 bit } m_axi_memQ_WDATA { O 64 vector } m_axi_memQ_WSTRB { O 8 vector } m_axi_memQ_WLAST { O 1 bit } m_axi_memQ_WID { O 1 vector } m_axi_memQ_WUSER { O 1 vector } m_axi_memQ_ARVALID { O 1 bit } m_axi_memQ_ARREADY { I 1 bit } m_axi_memQ_ARADDR { O 64 vector } m_axi_memQ_ARID { O 1 vector } m_axi_memQ_ARLEN { O 32 vector } m_axi_memQ_ARSIZE { O 3 vector } m_axi_memQ_ARBURST { O 2 vector } m_axi_memQ_ARLOCK { O 2 vector } m_axi_memQ_ARCACHE { O 4 vector } m_axi_memQ_ARPROT { O 3 vector } m_axi_memQ_ARQOS { O 4 vector } m_axi_memQ_ARREGION { O 4 vector } m_axi_memQ_ARUSER { O 1 vector } m_axi_memQ_RVALID { I 1 bit } m_axi_memQ_RREADY { O 1 bit } m_axi_memQ_RDATA { I 64 vector } m_axi_memQ_RLAST { I 1 bit } m_axi_memQ_RID { I 1 vector } m_axi_memQ_RFIFONUM { I 9 vector } m_axi_memQ_RUSER { I 1 vector } m_axi_memQ_RRESP { I 2 vector } m_axi_memQ_BVALID { I 1 bit } m_axi_memQ_BREADY { O 1 bit } m_axi_memQ_BRESP { I 2 vector } m_axi_memQ_BID { I 1 vector } m_axi_memQ_BUSER { I 1 vector } } \
} "
}

# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id 441 \
    name Q_DRAM \
    type other \
    dir I \
    reset_level 1 \
    sync_rst true \
    corename dc_Q_DRAM \
    op interface \
    ports { Q_DRAM { I 64 vector } } \
} "
}

# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id 442 \
    name zext_ln206 \
    type other \
    dir I \
    reset_level 1 \
    sync_rst true \
    corename dc_zext_ln206 \
    op interface \
    ports { zext_ln206 { I 3 vector } } \
} "
}

# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id 443 \
    name empty \
    type other \
    dir I \
    reset_level 1 \
    sync_rst true \
    corename dc_empty \
    op interface \
    ports { empty { I 3 vector } } \
} "
}

# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id 444 \
    name zext_ln206_1 \
    type other \
    dir I \
    reset_level 1 \
    sync_rst true \
    corename dc_zext_ln206_1 \
    op interface \
    ports { zext_ln206_1 { I 3 vector } } \
} "
}

# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id 445 \
    name xor_ln204 \
    type other \
    dir I \
    reset_level 1 \
    sync_rst true \
    corename dc_xor_ln204 \
    op interface \
    ports { xor_ln204 { I 3 vector } } \
} "
}

# Direct connection:
if {${::AESL::PGuard_autoexp_gen}} {
eval "cg_default_interface_gen_dc { \
    id -1 \
    name ap_ctrl \
    type ap_ctrl \
    reset_level 1 \
    sync_rst true \
    corename ap_ctrl \
    op interface \
    ports { ap_start { I 1 bit } ap_ready { O 1 bit } ap_done { O 1 bit } ap_idle { O 1 bit } } \
} "
}


# Adapter definition:
set PortName ap_clk
set DataWd 1 
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc cg_default_interface_gen_clock] == "cg_default_interface_gen_clock"} {
eval "cg_default_interface_gen_clock { \
    id -2 \
    name ${PortName} \
    reset_level 1 \
    sync_rst true \
    corename apif_ap_clk \
    data_wd ${DataWd} \
    op interface \
}"
} else {
puts "@W \[IMPL-113\] Cannot find bus interface model in the library. Ignored generation of bus interface for '${PortName}'"
}
}


# Adapter definition:
set PortName ap_rst
set DataWd 1 
if {${::AESL::PGuard_autoexp_gen}} {
if {[info proc cg_default_interface_gen_reset] == "cg_default_interface_gen_reset"} {
eval "cg_default_interface_gen_reset { \
    id -3 \
    name ${PortName} \
    reset_level 1 \
    sync_rst true \
    corename apif_ap_rst \
    data_wd ${DataWd} \
    op interface \
}"
} else {
puts "@W \[IMPL-114\] Cannot find bus interface model in the library. Ignored generation of bus interface for '${PortName}'"
}
}



# merge
if {${::AESL::PGuard_autoexp_gen}} {
    cg_default_interface_gen_dc_end
    cg_default_interface_gen_bundle_end
    AESL_LIB_XILADAPTER::native_axis_end
}


# flow_control definition:
set InstName top_flow_control_loop_pipe_sequential_init_U
set CompName top_flow_control_loop_pipe_sequential_init
set name flow_control_loop_pipe_sequential_init
if {${::AESL::PGuard_autocg_gen} && ${::AESL::PGuard_autocg_ipmgen}} {
if {[info proc ::AESL_LIB_VIRTEX::xil_gen_UPC_flow_control] == "::AESL_LIB_VIRTEX::xil_gen_UPC_flow_control"} {
eval "::AESL_LIB_VIRTEX::xil_gen_UPC_flow_control { \
    name ${name} \
    prefix top_ \
}"
} else {
puts "@W \[IMPL-107\] Cannot find ::AESL_LIB_VIRTEX::xil_gen_UPC_flow_control, check your platform lib"
}
}


if {${::AESL::PGuard_rtl_comp_handler}} {
	::AP::rtl_comp_handler $CompName BINDTYPE interface TYPE internal_upc_flow_control INSTNAME $InstName
}


