...
typedef struct {
    ...
    XTop_qr_uio_map maps[ MAX_UIO_MAPS ];
} XTop_qr_uio_info;

/***************** Variable Definitions **************************************/
static XTop_qr_uio_info uio_info;
...
static int uio_info_read_name(XTop_qr_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTop_qr_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTop_qr_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTop_qr_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTop_qr_Initialize(XTop_qr *InstancePtr, const char* InstanceName) {
	XTop_qr_uio_info *InfoPtr = &uio_info;
